...
if __name__ == "__main__":
    ...
    for r in range(1):
        ...
        for h in range(1,65):
            ...
            id1 = sort_id[group_flag[0]:group_flag[0]+20]
            id2 = sort_id[group_flag[1]-20:group_flag[1]]
            s1 = np.mean(deg[id1])
            s2 = np.mean(deg[id2])
            ...
            th = np.mean(sort_idk)-0.1

            if h>0:
                adj_mat = copy.deepcopy(adj)
                sim = embedding.dot(embedding.T)

                drop_li = np.where(sort_idk < th)[0]
                drop_li = [sort_id[i] for i in drop_li]
                adj_mat[drop_li] = 0
                adj_mat[:,drop_li] = 0
                for dl in drop_li:
                    neighborid = get_neighbors_all(G,dl,1)[1:]
                    simmm = sim[dl,neighborid]

                    # neighborid = neighborid[getListMaxNumIndex(simmm,topk=templ)]
                    adj_mat[dl,neighborid] = simmm
                ...
                #
                # adj_mat = copy.deepcopy(adj)
                # drop_li = np.where(sort_idk > th+0.2 )[0]
                # drop_li = [sort_id[i] for i in drop_li]
                # adj_mat[drop_li] = sim[drop_li]
                # adj_mat[:, drop_li] = sim[:, drop_li]
                # print(np.sum(adj_mat)/2)
                #
                # embedding2 = WL_noconcate_one(embedding, adj_mat)
                # embedding2 = preprocessing.normalize(embedding2, 'l2')
                # aaacc, nnnmi, fff1, para, predict_labels = cmd.sc_linear(embedding2, 1, num_of_class, true_labels)
                # print('+++++++++++: ACC:{:.6f}  NMI:{:.6f}  f1_macro:{:.6f}'.format(aaacc, nnnmi, fff1))
                # snmi = nmi+nnmi+nnnmi
                # embedding = (nmi/snmi)*embedding +(nnmi/snmi)*embedding1 +(nnnmi/snmi)*embedding2
