...
def load_data(dataset):
    ...
    # np.fill_diagonal(adj_mat, 0)
    # adj_mat = add_noise(adj_mat, true_labels,interclass_noise_rate,intraclass_noise_rate)
    deg = np.sum(adj_mat, axis=0)
...
def load_data_noise(dataset,interclass_noise_rate=0.1,intraclass_noise_rate=0.1):
    data =sio.loadmat('Data'+os.sep+dataset+'.mat')
    node_features = data['features']
    adj_mat = data['adj_mat']
    true_labels = data['labels'].reshape(-1)
    details = data['details'][0]
...
def add_noise(adj_mat, labels,interclass_noise_rate=0.1,intraclass_noise_rate=0.1):
    np.random.seed(2024)
    uclass = np.unique(labels)
    N = uclass.shape[0]
    for i in range(N):
        for j in range(i,N):
            label_i,label_j  = uclass[i],uclass[j]
            index_i = np.where(labels==label_i)[0]
            index_j = np.where(labels==label_j)[0]
            if i!=j: # interclass
                if interclass_noise_rate>0:
                    sampled_index_i = np.random.choice(index_i, int(len(index_i)*interclass_noise_rate), replace=False)
                    sampled_index_j = np.random.choice(index_j, int(len(index_j)*interclass_noise_rate), replace=False)
                    sampled_index_i = list(sampled_index_i)
                    sampled_index_j = list(sampled_index_j)
                    pairs = np.array(list(product(sampled_index_i, sampled_index_j)))
                    adj_mat[pairs[:,0],pairs[:,1]] = 1
                    adj_mat[pairs[:,1],pairs[:,0]] = 1
            else:# intraclass
                if intraclass_noise_rate>0:
                    sampled_index_i = np.random.choice(index_i, int(len(index_i)*intraclass_noise_rate), replace=False)
                    sampled_index_j = np.random.choice(index_j, int(len(index_j)*intraclass_noise_rate), replace=False)
                    pairs = np.array(list(product(sampled_index_i, sampled_index_j)))
                    adj_mat[pairs[:,0],pairs[:,1]] = 0
                    adj_mat[pairs[:,1],pairs[:,0]] = 0
    return adj_mat
